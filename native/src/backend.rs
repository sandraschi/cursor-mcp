...
const BACKEND_NAME: &str = "cursor-mcp-backend.exe";
const BACKEND_PORT: u16 = 11000;
const BACKEND_TAG: &str = "cursor-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "CURSOR_MCP_PORT";
const ENV_HOST: &str = "CURSOR_MCP_HOST";
const ENV_TAURI: &str = "CURSOR_MCP_TAURI";
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    log_line(
        &app,
        &format!("spawning {} (cwd {}) on port 11000",
    ...
    Ok(format!("Backend starting on port 11000"))
}
